function [puritylist,recoverylist,YALL1,YALL2,YALL3,YALL4]=Ze_GA_param(Yinit, opcond)
% ZE_GA_PARAM  Parameterized PSA simulator (CO2/N2 separation on IISERP-MOF).
%
% This function is identical to Ze_GA.m EXCEPT that the 7 operating
% conditions can be passed in as fields of the struct OPCOND. Any field
% that is missing falls back to the same hardcoded default that Ze_GA.m
% uses, so calling Ze_GA_param(Yinit, struct()) reproduces Ze_GA(Yinit)
% exactly.
%
% opcond fields (all optional):
%   tads_   - adsorption step time           (default 25)
%   vpress_ - pressurization velocity (m/s)  (default 0.2)
%   vads_   - adsorption velocity (m/s)      (default 0.5)
%   vblow_  - blowdown velocity (m/s)        (default 0.05)
%   vevac_  - evacuation velocity (m/s)      (default 1.0)
%   PINTc   - intermediate pressure ratio    (default 0.2)
%   PLc     - low pressure ratio             (default 0.1)
%
% Constraint: PLc < PINTc < 1 must hold (caller is responsible).
...
if nargin < 2, opcond = struct(); end

% ---- pull operating conditions from opcond, with defaults ------------
if isfield(opcond,'tads_'),   tads_   = opcond.tads_;   else, tads_   = 25;   end
if isfield(opcond,'vpress_'), vpress_ = opcond.vpress_; else, vpress_ = 0.2;  end
if isfield(opcond,'vads_'),   vads_   = opcond.vads_;   else, vads_   = 0.5;  end
if isfield(opcond,'vblow_'),  vblow_  = opcond.vblow_;  else, vblow_  = 0.05; end
if isfield(opcond,'vevac_'),  vevac_  = opcond.vevac_;  else, vevac_  = 1.0;  end
if isfield(opcond,'PINTc'),   PINTc   = opcond.PINTc;   else, PINTc   = 0.2;  end
if isfield(opcond,'PLc'),     PLc     = opcond.PLc;     else, PLc     = 0.1;  end
...
MW(1)=44.01; MW(2)=28.01;
AVGMW=sum(YFEED(1:ncomponent).*MW(1:ncomponent));
DM=1.6e-5;
DP=DM/TORT;
ROWG=(PH*AVGMW/1000)/(RG*T0);
ROWS=937.748616;
ROWW=7800.0;
CPG=1010.6;
CPS=1070.0;
...
MIU=1.72e-5;
KAPPA= 4.0/150.0*RP^2 * (BV^2/(1-BV)^2);
gamma=1.4;
eta=0.72;
gmin1byg=(gamma-1)/gamma;

%----IISERP-MOF dual-site equilibrium parameters----
B0(1)=9.3880523013e-08; B0(2)=2.549e-07;
D0(1)=5.22911980202e-07; D0(2)=2.549e-07;
U1(1)=-31135.10206;     U1(2)=-11890.66206;
U2(1)=-31135.10206;     U2(2)=-11890.66206;
...
qsat1 = 3.298453336;
qsat2 = 1.891959934;
QS1(1)= qsat1*ROWS; QS1(2)= qsat1*ROWS;
QS2(1)= qsat2*ROWS; QS2(2)= qsat2*ROWS;
QSref=QS1(2);
...
% ---- initial conditions from caller ------------
Y = Yinit;
Y(6*ngrid+1)=PH/PH;
Y(6*ngrid+2)=0;
Y(6*ngrid+3)=YFEED(1);
...
while swtch==0
    ...
    if n>5
        ...
        if length(ind)==5, swtch=1; end
    end
end

% ---- nested functions: ODE RHS + events + jacobians (UNCHANGED FROM Ze_GA.m) ----
    function yprime=Press1(time,y)
        ...
        ve(ngrid,size(y,2))=0;
        delz=1/ngrid;
        ve0=vpress; ve0=ve0.*(ve0>0);
        AVGMW=y(1:ngrid,:)*MW(1)+(1-y(1:ngrid,:))*MW(2);
        ROWG=(PH.*y(5*ngrid+(1:ngrid),:).*AVGMW/1000)./(RG*T0.*y(3*ngrid+(1:ngrid),:));
        PII5=CPG/RG .*PH./T0 .*AVGMW/1000;
        PII7=GRP1*(-CPA)*QSref.*AVGMW/1000;
        ...
        QbyC1(1:ngrid,:)=(RATIO1(1).*B0(1)*exp(-U1(1)./(RG*y(3*ngrid+(1:ngrid),:)*T0)))*QSref./DEN1 + (RATIO2(1).*D0(1)*exp(-U2(1)./(RG*y(3*ngrid+(1:ngrid),:)*T0)))*QSref./DEN2;
        QbyC2(1:ngrid,:)=(RATIO1(2).*B0(2)*exp(-U1(2)./(RG*y(3*ngrid+(1:ngrid),:)*T0)))*QSref./DEN1 + (RATIO2(2).*D0(2)*exp(-U2(2)./(RG*y(3*ngrid+(1:ngrid),:)*T0)))*QSref./DEN2;
        ...
        yprime(5*ngrid+(2:ngrid),:)=-y(3*ngrid+(2:ngrid),:)./delz.*(PE(2:ngrid,:).*ve(2:ngrid,:)./TE(2:ngrid,:)-PE(1:ngrid-1,:).*ve(1:ngrid-1,:)./TE(1:ngrid-1,:))+y(5*ngrid+(2:ngrid),:)./y(3*ngrid+(2:ngrid),:).*yprime(3*ngrid+(2:ngrid),:)-SHI*y(3*ngrid+(2:ngrid),:).*sum5(2:ngrid,:);
        yprime(5*ngrid+(1),:)=-y(3*ngrid+(1),:)/delz.*(PE(1,:).*ve(1,:)./TE(1,:)-Pin.*ve0./Tin)+y(5*ngrid+(1),:)./y(3*ngrid+(1),:).*yprime(3*ngrid+(1),:)-SHI*y(3*ngrid+(1),:).*sum5(1,:);
        sm1(1,:)=y(3*ngrid+(1),:)./y(5*ngrid+(1),:).*(1.0/delz^2).*(PE(1,:)./TE(1,:).*(y(2,:)-y(1,:)) -  2*Pin./Tin.*(y(1,:)-yin) );
        sm1(2:ngrid-1,:)=y(3*ngrid+(2:ngrid-1),:)./y(5*ngrid+(2:ngrid-1),:).*(1.0/delz^2).*(PE(2:ngrid-1,:)./TE(2:ngrid-1,:).*(y(3:ngrid,:)-y(2:ngrid-1,:)) -  PE(1:ngrid-2,:)./TE(1:ngrid-2,:).*(y(2:ngrid-1,:)-y(1:ngrid-2,:)) );
        sm1(ngrid,:)=y(3*ngrid+(ngrid),:)./y(5*ngrid+(ngrid),:).*(1.0/delz^2).*(PE(ngrid,:)./TE(ngrid,:).*(0) -  PE(ngrid-1,:)./TE(ngrid-1,:).*(y(ngrid,:)-y(ngrid-1,:)) );
        ...
        yprime(1:ngrid,:) = sm1/Pe-sm2all -SHI*y(3*ngrid+(1:ngrid),:)./y(5*ngrid+(1:ngrid),:).*yprime(ngrid+(1:ngrid),:) +y(1:ngrid,:)./y(3*ngrid+(1:ngrid),:).*yprime(3*ngrid+(1:ngrid),:) -y(1:ngrid,:)./y(5*ngrid+(1:ngrid),:).*yprime(5*ngrid+(1:ngrid),:);
    ...
    function yprime=Ads2(time,y)
        yprime=zeros(6*ngrid+2,size(y,2));
        ve(ngrid,size(y,2))=0;
        delz=1/ngrid;
        ve0=1;
        AVGMW=y(1:ngrid,:)*MW(1)+(1-y(1:ngrid,:))*MW(2);
        ...
        yin=(YFEED(1).*ve0*delz/2*Pe+y(1,:) )./ (1+ve0*delz/2*Pe);
        sum5=yprime(ngrid+(1:ngrid),:)+yprime(2*ngrid+(1:ngrid),:);
        Pin=y(5*ngrid+1,:)+ve0.*delz/2.0/GAMMA;
        P0=(2.0*Pin-y(5*ngrid+1,:));
        ryplus(1,:)=(y(5*ngrid+1,:)-P0(1,:)+10e-10)./(y(5*ngrid+2,:)-y(5*ngrid+1,:)+10e-10);
        ryplus(2:ngrid-1,:)=(y(5*ngrid+(2:ngrid-1),:)-y(5*ngrid+(1:ngrid-2),:)+10e-10)./(y(5*ngrid+(3:ngrid),:)-y(5*ngrid+(2:ngrid-1),:)+10e-10);
        ryplus(ngrid,:)=(y(5*ngrid+ngrid,:)-y(5*ngrid+ngrid-1,:)+10e-10)./(2.0*(1.0-y(5*ngrid+ngrid,:))+10e-10);
        limy=(ryplus+abs(ryplus))./(1+abs(ryplus));
        Fye(5*ngrid+(1:ngrid-1),:)=0.5.*limy(1:ngrid-1,:).*(y(5*ngrid+(2:ngrid),:)-y(5*ngrid+(1:ngrid-1),:));
        Fye(5*ngrid+ngrid,:)=1.*(1.0-y(5*ngrid+ngrid,:));
        PE(1:ngrid,:)=y(5*ngrid+(1:ngrid),:)+Fye(5*ngrid+(1:ngrid),:);
        y0(1,:)=(2*yin-y(1,:));
        ryplus(1,:)=(y(1,:)-y0+10e-10)./(y(2,:)-y(1,:)+10e-10);
        ryplus(2:ngrid-1,:)=(y(2:ngrid-1,:)-y(1:ngrid-2,:)+10e-10)./(y(3:ngrid,:)-y(2:ngrid-1,:)+10e-10);
        limy=(ryplus+abs(ryplus))./(1+abs(ryplus));
    ...
    function yprime=Blow3(time,y)
        yprime=zeros(6*ngrid+3,size(y,2));
        ve(ngrid,size(y,2))=0;
        delz=1/ngrid;
        ...
        ryplus(1,:)=(y(5*ngrid+1,:)-P0(1,:)+10e-10)./(y(5*ngrid+2,:)-y(5*ngrid+1,:)+10e-10);
        ryplus(2:ngrid-1,:)=(y(5*ngrid+(2:ngrid-1),:)-y(5*ngrid+(1:ngrid-2),:)+10e-10)./(y(5*ngrid+(3:ngrid),:)-y(5*ngrid+(2:ngrid-1),:)+10e-10);
        ryplus(ngrid,:)=(y(5*ngrid+ngrid,:)-y(5*ngrid+ngrid-1,:)+10e-10)./(2.0*(PBARBED-y(5*ngrid+ngrid,:))+10e-10);
        limy=(ryplus+abs(ryplus))./(1+abs(ryplus));
        Fye(5*ngrid+(1:ngrid-1),:)=0.5.*limy(1:ngrid-1,:).*(y(5*ngrid+(2:ngrid),:)-y(5*ngrid+(1:ngrid-1),:));
        Fye(5*ngrid+ngrid,:)=1.*(PBARBED-y(5*ngrid+ngrid,:));
        PE(1:ngrid,:)=y(5*ngrid+(1:ngrid),:)+Fye(5*ngrid+(1:ngrid),:);
        y0(1,:)=(2*yin-y(1,:));
        ryplus(1,:)=(y(1,:)-y0+10e-10)./(y(2,:)-y(1,:)+10e-10);
        ryplus(2:ngrid-1,:)=(y(2:ngrid-1,:)-y(1:ngrid-2,:)+10e-10)./(y(3:ngrid,:)-y(2:ngrid-1,:)+10e-10);
    ...
    function [value,isterminal,direction] = events(time,y)
        value=y(5*ngrid+1)-1; isterminal=1; direction=1;
    end
    function [value,isterminal,direction] = events2(time,y)
        value=y(5*ngrid+ngrid)-PINT/PH; isterminal=1; direction=-1;
    end
    function [value,isterminal,direction] = events3(time,y)
        value=y(5*ngrid+ngrid)-PL/PH; isterminal=1; direction=-1;
    end
